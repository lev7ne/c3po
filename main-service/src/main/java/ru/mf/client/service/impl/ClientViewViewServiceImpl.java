...
public class ClientViewViewServiceImpl implements ClientViewService<ClientViewDto> {
    ...
    @Transactional
    @Override
    public ClientViewDto saveClient(ClientViewDto dto) {
        var client = clientMapper.toEntity(dto);

        return clientMapper.toViewDto(clientRepository.save(client));
    }

    @Transactional
    @Override
    public ClientViewDto updateClient(ClientViewDto dto) {
        var existingClient = clientRepository.findById(dto.getId())
                .orElseThrow(() -> new NotFoundException("Client with id=" + dto.getId() + " was not found"));

        clientMapper.update(dto, existingClient);
        var updatedClient = clientRepository.save(existingClient);

        return clientMapper.toViewDto(updatedClient);
    }
